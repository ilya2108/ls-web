  ...
  const filterRows = (rows) => {
    return rows.filter((row) => {
      return inputVal.match(/( *\[.*?] *)+/g)
        ? inputVal
            .match(/\[(.*?)]/g)
            .every((tag) => row[0].toLowerCase().includes(tag.toLowerCase()))
        : row[0].toLowerCase().includes(inputVal.toLowerCase());
    });
  };

  const tableRows = filterRows(props.data.rows).map((row, i) => {
    return {
      cells: row.map((rowItem, index) => ({
        ...
        content:
          index === 0 && props.links ? (
            <Link
              href={
                props.links.urlPrefix +
                `${encodeURIComponent(props.links.nameId[i])}`
              }
            >
              <a>{rowItem}</a>
            </Link>
